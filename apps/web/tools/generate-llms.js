...
function main() {
	...
	// Some single-page sites keep a placeholder file in src/pages while the
	// actual metadata lives in App.jsx. Fall back to App so a placeholder does
	// not prevent the production build from running.
	if (pages.length === 0) {
		const appPage = processPageFile(appJsxPath, new Map());
		pages = appPage ? [appPage] : [];
	}
